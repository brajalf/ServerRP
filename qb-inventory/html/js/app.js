...
// Fallback para no petar si la carga async de traducciones llega tarde o falla
window.translations = (window.translations && typeof window.translations === 'object') ? window.translations : {};
const T = (k, fb) => (window.translations && window.translations[k]) ?? fb ?? k;

// Intento de carga (si tu UI ya lo hace en otro sitio, esto no molesta)
(async () => {
  try {
    const res = await $.post('https://qb-inventory/GetTranslate', JSON.stringify({}));
    if (res && typeof res === 'object') window.translations = res;
  } catch (e) { /* dejamos el fallback */ }
})();
// GET TRANSLATE
let translate;
...
function FormatItemInfo(itemData) {
  ...
  if (!itemData) {
    $('.item-info-title').html('<p></p>');
    $('.item-info-description').html('<p></p>');
    return;
  }
  const info = itemData.info || {};

  if (itemData.name === 'id_card') {
    let gender = T('Man', 'Hombre');
    if (N(info.gender) === 1) gender = T('Woman', 'Mujer');

    $('.item-info-title').html('<p>' + S(itemData.label) + '</p>');
    $('.item-info-description').html(
      '<p><strong>CSN: </strong><span>' + S(info.citizenid) + '</span></p>' +
      `<p><strong>${T('FirstName','Nombre')}: </strong><span>${S(info.firstname)}</span></p>` +
      `<p><strong>${T('LastName','Apellido')}: </strong><span>${S(info.lastname)}</span></p>` +
      `<p><strong>${T('Birthday','Fecha de nacimiento')}: </strong><span>${S(info.birthdate)}</span></p>` +
      `<p><strong>${T('Gender','Género')}: </strong><span>${S(gender)}</span></p>` +
      `<p><strong>${T('Nacionality','Nacionalidad')}: </strong><span>${S(info.nationality)}</span></p>`
    );
    ...
    $('.item-info-title').html('<p>' + S(itemData.label) + '</p>');
    $('.item-info-description').html(
      '<p><strong>Pass-ID: </strong><span>' + S(info.id) + '</span></p>' +
      `<p><strong>${T('FirstName','Nombre')}: </strong><span>${S(info.firstname)}</span></p>` +
      `<p><strong>${T('LastName','Apellido')}: </strong><span>${S(info.lastname)}</span></p>` +
      '<p><strong>CSN: </strong><span>' + S(info.citizenid) + '</span></p>'
    );
    ...
    $('.item-info-title').html('<p>' + S(itemData.label) + '</p>');
    $('.item-info-description').html('<p>' + N(info.uses) + ' uses left.</p>');

  } else if (itemData.type === 'weapon') {
    $('.item-info-title').html('<p>' + S(itemData.label) + '</p>');

    const ammo = N(info.ammo);
    if (Array.isArray(info.attachments) && info.attachments.length) {
      let attachmentString = info.attachments.map(a => S(a && a.label)).filter(Boolean).join(', ');
      $('.item-info-description').html(`
        <div class="weapon-desc-info">
          <p><strong>${T('SerialNumber','N° de serie')}:</strong> <span>${S(info.serie)}</span></p>
          <p><strong>${T('Ammo','Munición')}:</strong> <span>${ammo}</span></p>
        </div>
        <p><strong>${T('Accesories','Accesorios')}:</strong> <span>${S(attachmentString)}</span></p>
      `);
    } else {
      $('.item-info-description').html(`
        <div class="weapon-desc-info">
          <div class="d-flex align-items-center justify-content-between">
            <div class="d-flex align-items-center">
              ...
              <div class="weapon-number">${S(info.serie)}</div>
            </div>
            <div class="d-flex align-items-center">
        ...
        <p>${S(itemData.description)}</p>
      `);
    }

  } else if (info.costs != null && info.costs != undefined) {
    $('.item-info-title').html('<p>' + S(itemData.label) + '</p>');
    $('.item-info-description').html('<p>' + S(info.costs) + '</p>');

  } else if (itemData.name === 'stickynote') {
    $('.item-info-title').html('<p>' + S(itemData.label) + '</p>');
    $('.item-info-description').html('<p>' + S(info.label) + '</p>');

  } else if (itemData.name === 'stickynotespack') {
    $('.item-info-title').html('<p>' + S(itemData.label) + '</p>');
    $('.item-info-description').html(
      `<p><strong>${T('RemaingNotes','Notas restantes')}: </strong>${N(info.notes)}</p>`
    );

  } else if (itemData.name === 'moneybag') {
    $('.item-info-title').html('<p>' + S(itemData.label) + '</p>');
    $('.item-info-description').html(
      `<p><strong>${T('EfectiveQuantity','Cantidad de efectivo')}: </strong><span>$${N(info.cash)}</span></p>`
    );

  } else if (itemData.name === 'customs_receipt') {
    $('.item-info-title').html('<p>' + S(itemData.label) + '</p>');
    $('.item-info-description').html(
      '<p><strong>Primary Color: </strong><span>' + S(info.primaryColor) + '</span></p>' +
      '<p><strong>Secondary Color: </strong><span>' + S(info.secondaryColor) + '</span></p>' +
      '<p><strong>Pearlescent Color: </strong><span>' + S(info.pearlescentColour) + '</span></p>' +
      '<p><strong>Wheel Model: </strong><span>' + S(info.wheelName) + '</span></p>' +
      '<p><strong>Wheel Color: </strong><span>' + S(info.wheelColor) + '</span></p>' +
      '<p><strong>Wheel Smoke Color: </strong><span>' + S(info.wheelSmokeColor) + '</span></p>' +
      '<p><strong>Dashboard Color: </strong><span>' + S(info.dashboardColor) + '</span></p>' +
      '<p><strong>Interior Color: </strong><span>' + S(info.interiorColor) + '</span></p>' +
      '<p><strong>Neon: </strong><span>' + S(info.neonColor) + '</span></p>' +
      '<p><strong>Xenon: </strong><span>' + S(info.xenonColor) + '</span></p>' +
      '<p><strong>Window tint: </strong><span>' + S(info.windowTint) + '</span></p>' +
      '<p><strong>Cosmetics: </strong><span>' + S(info.vehicleCosmetics) + '</span></p>'
    );

  } else if (itemData.name === 'markedbills') {
    $('.item-info-title').html('<p>' + S(itemData.label) + '</p>');
    $('.item-info-description').html(`<p>${T('MarkedMoney','Dinero marcado')}</p>`);

  } else if (itemData.name === 'labkey') {
    $('.item-info-title').html('<p>' + S(itemData.label) + '</p>');
    $('.item-info-description').html('<p>Lab: ' + S(info.lab) + '</p>');

  } else if (itemData.name === 'placa_lspd' || itemData.name === 'placa_bcsd') {
    $('.item-info-title').html('<p>' + S(itemData.label) + '</p>');
    $('.item-info-description').html(
      '<p><strong>Rango: </strong><span>' + S(info.grade) + '</span></p>' +
      '<p><strong>Nº Placa: </strong><span>' + S(info.n) + '</span></p>'
    );
    ...
    if (info.serie) {
      $('.item-info-description').html(
        '<p>' + S(itemData.description) + '</p><b>Nº Serie: </b>' + S(info.serie)
      );
    } else {
      $('.item-info-description').html('<p>' + S(itemData.description) + '</p>');
    }
  }
}
